...
plot_creation <- function(data_gene, gene_list, covariate, categorical_covariate_list) {
    ...
    linked_data1 <- data.frame(
        ...
        "charlson_score" = strtoi(meta_data$charlson_score),
...
male_table <- linked_remove_null1 %>% filter(Categorical2 == " male")
female_table <- linked_remove_null1 %>% filter(Categorical2 == " female")

table_summary <- data.frame("Variable" = c("Age: mean (sd)", "Mechanical Ventilation: n(%)", "Yes", "No",
                                           "Follow Up Days: mean (sd)", "ICU Status: n(%)", "Yes", "No", "COVID-19 Status: n(%)",
                                           "COVID-19", "NON COVID-19", "Charlson Score: mean (sd)"),
                     "Value" = c(paste0(round(mean(male_table$Age),2)," (", round(sd(male_table$Age),2),")"),"",
                                 round(sum(male_table$Categorical == " yes")/length(male_table[,1])*100,0),
                                 round(sum(male_table$Categorical == " no")/length(male_table[,1])*100,0),
                                 paste0(round(mean(male_table$Days),2)," (", round(sd(male_table$Days),2),")"), "",
                                 round(sum(male_table$Categorical3 == " yes")/length(male_table[,1])*100,0),
                                 round(sum(male_table$Categorical3 == " no")/length(male_table[,1])*100,0), "",
                                 round(sum(male_table$Categorical4 == "disease state: COVID-19")
                                       /length(male_table[,1])*100,0),
                                 round(sum(male_table$Categorical4 == "disease state: non-COVID-19")
                                       /length(male_table[,1])*100,0),
                                 paste0(round(mean(male_table$charlson_score),2)," (", round(sd(male_table$charlson_score),2),")")),
                     "Variable" = c("Age: mean (sd)", "Mechanical Ventilation: n(%)", "Yes", "No",
                                           "Follow Up Days: mean (sd)", "ICU Status: n(%)", "Yes", "No", "COVID-19 Status: n(%)",
                                           "COVID-19", "NON COVID-19", "Charlson Score: mean (sd)"),
                     "Value" = c(paste0(round(mean(female_table$Age),2)," (", round(sd(female_table$Age),2),")"),"",
                                 round(sum(female_table$Categorical == " yes")/length(female_table[,1])*100,2),
                                 round(sum(female_table$Categorical == " no")/length(female_table[,1])*100,2),
                                 paste0(round(mean(female_table$Days),2)," (", round(sd(female_table$Days),2),")"), "",
                                 round(sum(female_table$Categorical3 == " yes")/length(female_table[,1])*100,2),
                                 round(sum(female_table$Categorical3 == " no")/length(female_table[,1])*100,2), "",
                                 round(sum(female_table$Categorical4 == "disease state: COVID-19")
                                       /length(female_table[,1])*100,2),
                                 round(sum(female_table$Categorical4 == "disease state: non-COVID-19")
                                       /length(female_table[,1])*100,2),
                                 paste0(round(mean(female_table$charlson_score),2)," (", round(sd(female_table$charlson_score),2),")")))

# Edit table attributes
kable(table_summary, caption = "<span style='font-size:20px'><b>Summary Table for Covariates</b></span>",
    col.names = c("Male", sum(linked_remove_null1$Categorical2 == " male"), "Female", sum(linked_remove_null1$Categorical2 == " female")),
    align = c('l','r'),escape = T) %>%
    row_spec(c(0,1,2,5,6,9,12),bold=T,hline_after = T)%>%
    add_indent(positions = c(3,4,7,8,10,11)) %>%
    column_spec(2,bold = T, background = "#ADD8E6")%>%
    column_spec(1, background = "yellow")%>%
    column_spec(4,bold = T, background = "#ADD8E6")%>%
